...
public class UserViewTimesJob extends Configured implements Tool {

	@Override
	public int run(String[] arg0) throws Exception {
		final Job job = new Job(new Configuration(),
				UserViewTimesJob.class.getSimpleName());
		job.setJarByClass(UserViewTimesJob.class);
		FileInputFormat.setInputPaths(job, arg0[0]);
		job.setMapperClass(CountEveryUseriViewtimesMaper.class);
		job.setCombinerClass(IntSumReducer.class);

		job.setReducerClass(UserViewTimeCountReduce.class);
	...
	public static void main(String[] args) {
		String[] agrs = new String[] { "file:///E:/yearout",
				"file:///E:/userviewtimeseveryday" };
		try {
			int res = ToolRunner.run(new Configuration(), new UserViewTimesJob(),
					agrs);
			System.exit(res);
		} catch (Exception e) {
